...
- Use Trello REST API directly with `.env.trello`; never use the Trello MCP for this project.
- Run SDD in automatic mode and save phase artifacts to Engram using `sdd/{ticket-id}/...` topic keys.
- Runtime gates are blocking: no verified checkout runtime means no tests, commit, push, or PR.
...
1. Load required rules: ticket-router, branch-first, checkout-runtime-gate, pre-commit-gate, commit-language, test-mandate, plus package-specific rules.
2. Select/confirm Trello ticket, move it to In Progress, and comment with the branch name.
3. From repo root, update `develop`, create/switch to `feature/{TICKET-ID}-{desc}` or `fix/{TICKET-ID}-{desc}`, then set up and verify checkout runtime.
